...
import { askAi } from "../services/openRouter.servics.js"
...
export const analyzeResume = async (req, res) => {
    try {
        ...
        const messages = [
            {
                ...
                Return ONLY raw JSON, with no markdown formatting, no code fences (no triple backticks), no explanation text before or after.
        ...
        // Markdown code fences hatao agar AI ne accidentally add kar diye ho
...
export const generateQuestion=async (req,res)=>{
    try{
            ...
            if(!userPrompt.trim()){
                return res.status(400).json({message:"user prompt is empty"})
            }
            const messages=[
                {
                    role:"system",
...
Make questions based on the candidate’s role, experience,interviewMode, projects, skills, and resume details.
    ...
    .slice(0,5);
     if(questionsArray.length===0){
       return  res.status(500).json({message:"AI faild to generate question"})
     }
     user.credits-=50;
     await user.save()
...
export const submitAnswer=async(req,res)=>{
    try{
        ...
        if(timeTaken>question.timeLimit){
            question.score=0;
            question.feedback="You took too long to answer this question";
            question.answer=""
            await interview.save();
            return res.json({
                feedback:question.feedback
...
export const finishInterview=async(req,res)=>{
    try{
        ...
        let totalCorrectneess=0;
        interview.questions.forEach((q)=>{
            totalScore+=q.score||0;
            totalConfidence+=q.confidence||0;
            totalCommunication+=q.communication||0;
            totalCorrectneess+=q.correctness||0;
        })
        const finalScore=totalQuestions?totalScore/totalQuestions:0;
        const avgConfidence=totalQuestions?totalConfidence/totalQuestions:0;
        const avgCommunication=totalQuestions?totalCommunication/totalQuestions:0;
        const avgCorrectness=totalQuestions?totalCorrectneess/totalQuestions:0;
