...
const Navbar = () => {
  ...
  return (
    <div className='bg-[#f3f3f3] flex justify-center px-44 pt-6'>
        <motion.div 
        initial={{ opacity: 0, y: -40 }} 
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 1.05, delay: 0.2 }}
        className='w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4  flex i justify-between items-center relative'>
             ...
             <div className='relative'>
                ...
                {showUserPopup && (
                    <div className='absolute right-0 mt-3 w-48 bg-white shadow-xl border border-gray-200 rounded-xl p-4 z-50'>
                        ...
                        <button onClick={handleLogout} className='w-full text-left text-sm py-2 flex items-center gap-2 text-red-500'> <HiOutlineLogout size={18} />Logout</button>
                    </div>
                )}
            </div>
        </div>
